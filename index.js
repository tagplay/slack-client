...
SlackAPI.prototype.getPaginated = async function getPaginated (path, params, token) {
  ...
  if (result.response_metadata && result.response_metadata.next_cursor) {
    const [nextErr, nextPage] = await this.getPaginated(path, {...params, cursor: result.response_metadata.next_cursor}, token)
...
SlackAPI.prototype.filePublicUrl = async function filePublicUrl (fileId) {
  ...
  return this.post('files.sharedPublicURL', {
    file: fileId
...
SlackAPI.prototype.getPermalink = async function getPermalink (channelId, messageId) {
...
SlackAPI.prototype.postMessage = async function postMessage (channel, text, mrkdwn, attachments) {
